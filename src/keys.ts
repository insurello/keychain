...
export const decode = (data: any): KeySet<jwk.Key> => {
  if (
    ...
    const keys: Set<jwk.Key> = data.keys
      .filter(jwk.isKey)
      .map(jwk.decode)
      .reduce((set: Set<jwk.Key>, key: jwk.Key) => set.add(key), new Set());
...
export const selectKey = (kid: string) => <T extends KeySetEntry>(
  keyOrSet: T | KeySet<T>
): T => {
...
const isKeySet = <T extends KeySetEntry>(
  key: T | KeySet<T>
): key is KeySet<T> => (key as KeySet<T>).keys !== undefined;
